...
class CityMap:
    ...
    def search(self, start, goal):
        """Breadth-first search. Return the answer as a linked list of States
        ...
        while queue:
            ...
            current_stop = current_state.get_stop()

            if current_stop == goal:
                return current_state

            for neighbor in self.get_neighbors_codes(current_stop):
                if neighbor not in visited:
                    visited.add(neighbor)
                    queue.append(State(neighbor, current_state))
